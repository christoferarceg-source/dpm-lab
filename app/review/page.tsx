...
export default function ReviewPage() {
  ...
  return (
    <div className="max-w-xl space-y-4">
      ...
      <div className="bg-surface border border-border rounded-xl p-6 min-h-[220px] flex flex-col">
        <p className="text-xs uppercase tracking-wide text-muted mb-3">{flipped ? "Answer" : "Question"}</p>
      ...
      {!flipped ? (
        <button
          ...
          className="w-full px-4 py-3 rounded-md text-sm font-medium bg-accent text-accent-fg hover:opacity-90"
        >
          Show answer
        </button>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {GRADES.map((g) => (
            <button
              ...
              className={`px-3 py-2.5 rounded-md text-sm border ${g.cls} hover:opacity-90`}
            >
