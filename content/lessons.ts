...
const u1: Lesson[] = [
  ...
  lesson("u1-dp1", 1, "What a data product is (and isn't)", [
    concept(
      "Curated, reliable, reusable, intentional",
      "A **data product** is a curated, reliable, reusable data asset designed for *ongoing* use: a defined domain boundary, a stable model, embedded validation, clear access. It's not a **dataset** (storage isn't productisation), not a **dashboard** (a consumption layer), not a **pipeline** (movement isn't ownership). It needs stable semantics, embedded governance, and a defined consumer purpose.",
      "what-is-a-data-product"
    ),
    match("Match the thing to why it is *not* a data product.", [["A dataset", "Storage alone: no stewardship or service expectations"], ["A dashboard", "A consumption layer: interprets, doesn't govern the logic"], ["A pipeline", "Movement isn't ownership: no durable quality guarantee"]], "Each one is useful. None of them carries ownership, service expectations, and embedded quality by itself.", "what-is-a-data-product"),
    mcq("Raj's pipeline lands a deals table every night at 02:00 and it's been reliable for a year. Is that a data product?", ["Yes, it's reliable and reusable", "Not yet: nobody owns its definitions, there are no service expectations or quality controls, and no stated consumer", "Yes, because a dashboard reads it", "No, because it isn't real-time"], 1, "Reliability of the job isn't the test. A product has an owner, contracts, SLOs, and a consumer purpose. The table is the raw material for one.", ["Reliable motion is still motion.", undefined, "The dashboard is a consumer, not evidence of productisation.", "Latency isn't the criterion."], "what-is-a-data-product"),
    tf("A well-designed dashboard with consistent metrics is itself the data product.", false, "The dashboard is a consumption layer. The product is the governed logic beneath it: definitions, access, metrics that many dashboards can depend on.", "what-is-a-data-product"),
    fill("Three forces make a data product: stable semantics, embedded ___, and a defined consumer purpose.", ["governance", "dashboards", "storage", "marketing"], "governance", "Access controls, privacy, and regulatory alignment built in structurally, not added later.", "what-is-a-data-product"),
    mcq("What changes when data becomes a product rather than a by-product of systems?", ["It gets a nicer chart", "The focus shifts from delivering outputs to maintaining dependable infrastructure others rely on repeatedly", "It moves to the cloud", "It needs fewer engineers"], 1, "That's the definition of decision infrastructure: something people can build on again and again without re-checking it.", undefined, "what-is-a-data-product"),
  ]),
  lesson("u1-dp2", 1, "Anatomy, core, and types", [
    concept(
      "What's inside, and the three kinds",
      "**Anatomy**: input contracts, transformations & semantics, quality signals, metadata, embedded governance. A **canonical core** (authoritative entities and metrics) with **output projections** per consumer: an API for operations, tables for analytics, auditable extracts for compliance. The core never changes; projections adapt. **Types**: source-aligned (expose a domain faithfully), aggregate (encode cross-domain logic like how revenue is calculated), consumer-aligned (built for one use case's latency, interface, and guarantees).",
      "what-is-a-data-product"
    ),
    match("Match the anatomy part to its job.", [["Input contracts", "Schemas and validation against upstream volatility"], ["Transformations & semantics", "Encoded rules that prevent interpretive drift"], ["Quality signals", "Continuous monitoring, measurable guarantees"], ["Metadata", "Documentation, lineage, catalogue: discoverability"]], "Five parts plus output ports. Each one answers a different way a product can silently break.", "what-is-a-data-product"),
    order("Order the product types from closest-to-source to closest-to-decision.", ["Source-aligned", "Aggregate", "Consumer-aligned"], "Consumer-aligned products depend on aggregate ones, which depend on source-aligned ones. Break the chain and the top-level numbers drift.", "what-is-a-data-product"),
    mcq("Which type *encodes how revenue is calculated and how KPIs roll up across segments*?", ["Source-aligned", "Aggregate", "Consumer-aligned", "A pipeline"], 1, "Aggregate products consolidate logic from several domains into standardised measures. Their reliability is what people mean by 'trust in the metrics'.", undefined, "what-is-a-data-product"),
    tf("When compliance needs a different extract, you change the canonical core's definition of revenue.", false, "You add or change a *projection*. The core definition stays put; that's the whole point of separating them.", "what-is-a-data-product"),
    mcq("Meridian's Sales Funnel Accelerator dashboard, the metric tree behind it, and the raw CRM extract: what are they?", ["All three are consumer-aligned products", "Dashboard = consumer-aligned projection; metric tree = aggregate product; CRM extract = source-aligned (once it has a contract and owner)", "All three are pipelines", "Dashboard = aggregate; tree = source-aligned; extract = consumer-aligned"], 1, "Map what you build to the type, and you know what each one must guarantee: fidelity at the source, semantic discipline in the aggregate, service levels at the consumer.", undefined, "what-is-a-data-product"),
  ]),
